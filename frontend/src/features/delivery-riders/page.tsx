...
export default function DeliveryRidersPage() {
  ...
  const [deletingRider, setDeletingRider] = useState<DeliveryRider | null>(null);

  const { data: riders, isLoading } = useDeliveryRiders();
  const createMutation = useCreateDeliveryRider();
  const updateMutation = useUpdateDeliveryRider();
  const deleteMutation = useDeleteDeliveryRider();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<DeliveryRiderFormData>({
    resolver: zodResolver(deliveryRiderSchema(tVal)),
    defaultValues: {
      name: "",
      phone: "",
      notes: undefined,
  ...
  const openEditDialog = (rider: DeliveryRider) => {
    ...
    reset({ name: rider.name, phone: rider.phone, notes: rider.notes ?? undefined });
  };
